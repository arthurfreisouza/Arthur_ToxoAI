...
- Q: After the notebook fine-tunes Llama 3.2 1B locally on your laptop, should the trained model still be pushed to the private Hugging Face repository and served from the scale-to-zero Inference Endpoint, as currently planned? → A: Yes — serving is unchanged. Training moves to the Administrator's laptop GPU inside a Jupyter notebook; the notebook and the saved model live in `training_model/` at the repository root; the finished weights are then pushed to the private Hugging Face repository and served exactly as before (FR-097, FR-098).
- Q: What should the `training_model/` folder contain — only the fine-tuning notebook and the locally saved model, or the whole offline model strand (split generation, dataset writer, replay and held-out evaluation too)? → A: Notebook and saved model only. The versioned corpus split, the dataset writer, the historical replay, the held-out evaluation, and the benchmark remain command-line scripts in `eval/`; the notebook consumes the fixed split artefact and does not regenerate it.
- Q: How much VRAM does your laptop's GPU have (or what GPU model is it)? → A: An NVIDIA GeForce RTX 2070 Mobile (Predator PH315-53), 8 GB of VRAM. Standard LoRA in fp16 on Llama 3.2 1B fits within that budget.
- Q: Does "the fine-tuned model must be Llama 3.2 1B" fix the model at 1B permanently, or does the 1B → 3B → 8B escalation ladder remain if the release gates fail? → A: The ladder remains. 1B is the trained target; escalation is the sanctioned remedy only if SC-015 or SC-016 fail. FR-084 is unchanged.
...
- **FR-043**: System MUST classify the patient described, returning a maternal classification, a child classification, an argumentation, and a recommendation, each drawn from the defined value sets in FR-064 and FR-065.
...
- **FR-062**: System MUST record, for every classification it issues, the configuration version that produced it — the model, prompt, and canonical-text lookup versions — so any result can be reproduced and audited after the fact.
...
- **FR-105**: System MUST fine-tune Llama 3.2 1B Instruct **locally, on the Administrator's laptop GPU** (NVIDIA GeForce RTX 2070 Mobile, 8 GB VRAM), inside a Jupyter notebook stored in a `training_model/` folder at the repository root. No cloud or hosted training service is used; only the finished weights leave the machine.
- **FR-106**: The notebook MUST save the fine-tuned model into `training_model/`, and those saved weights MUST be excluded from git — they derive from clinical records and fall under FR-097's prohibition on publication. From `training_model/` the weights are pushed to the private Hugging Face repository and served per FR-098; where training runs changes nothing about how the model is served.
- **FR-107**: The notebook MUST consume the fixed, versioned corpus split artefact and the training dataset produced by the offline tooling (FR-104), and MUST NOT regenerate the split or the dataset itself. The historical replay (SC-015), held-out evaluation (SC-023), and comparative benchmark (SC-022) remain command-line scripts outside `training_model/`.
...
- **FR-070**: System MUST record with each captured row the timestamp, the configuration version per FR-062 (model, prompt, and canonical-text lookup versions), and the identifier of the account that submitted it.
...
- **SC-017**: 100% of classification events are captured to the training dataset with their configuration version (model, prompt, and canonical-text lookup versions), and the captured file remains loadable alongside the historical dataset without transformation.
...
- The base model is **Llama 3.2 1B Instruct**, fine-tuned on the project dataset and served from **Hugging Face**. 1B is the owner's target for cost reasons. If the fine-tuned 1B model cannot reproduce all 24 historical classifications (SC-015) or cannot classify identical findings identically (SC-016), the response is to **escalate the base model — 1B → 3B → 8B** — and absorb the higher hosting cost, not to lower the gate or to hand classification to a rules engine. (Reconfirmed 2026-08-07: 1B is the trained target and the ladder stays.)
- Fine-tuning runs **locally on the owner's laptop GPU** — an NVIDIA GeForce RTX 2070 Mobile with 8 GB of VRAM — through a Jupyter notebook kept, together with the saved model, in `training_model/` at the repository root (FR-105, FR-106). The saved weights are git-ignored, then pushed to the private Hugging Face repository for serving. An 8 GB card fits standard LoRA in fp16 at 1B; QLoRA is the fallback if escalation to 3B is ever needed. **Known decision point**: the FR-084 ladder ends at 8B, and an 8B fine-tune generally does not fit 8 GB of VRAM even with QLoRA — if escalation ever reaches 8B, FR-105's local-only constraint must be revisited by the owner (new hardware or an explicit exception) rather than silently broken.
...
- Fine-tuning runs locally on the owner's laptop (FR-105) and therefore incurs no cloud training cost at all; baseline benchmark spend remains a one-off incurred outside the running system. Neither is part of the recurring ceiling in SC-014.
